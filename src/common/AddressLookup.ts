...
export class AddressLookup {
  ...
  lookupCoordinates(suggestion, addToPrevious = true) {
    ...
    return this.$http.get(`https://geocode.arcgis.com/arcgis/rest/services/World/GeocodeServer/findAddressCandidates?&Address=${suggestion.text}&State=TX&f=json&City=Houston&maxLocations=10&maxResultSize=1&outFields=StreetType&magicKey=${suggestion.magicKey}&category=&location=-95.3632700,29.7632800&distance=10000&f=pjson`)
      .map(res => res.json())
      .toPromise()
  ...
  lookupAddress(address) {
    ...
    return this.$http.get(`https://geocode.arcgis.com/arcgis/rest/services/World/GeocodeServer/suggest?text=${address}&category=&location=-95.3632700,29.7632800&distance=10000&f=pjson`)
      .map(res => res.json())
      .toPromise()
